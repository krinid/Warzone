...
function Client_PresentPlayCardUI (game, cardInstance, playCard)
	...
	if (game.Us == nil) then return; end --technically not required b/c spectators could never initiate this function (requires playing a Card, which they can't do b/c they're not in the game)
...
function play_Critters_card (game, cardInstance, playCard)
	...
	local arrValidTerrs = getTerritoriesWithinDistanceFromAPlayerBelongingToAnotherPlayer (game, intPlayerID_cardPlayer, -1, Mod.Settings.CrittersRange or 2);
	...
	TargetPlayerLabel = UI.CreateLabel (horzTargetPlayer).SetText ("Player to target: " ..assignToPlayerName);
	UI.CreateLabel (vert).SetText ("   Select the player to target");
	UI.CreateLabel (vert).SetText ("_").SetColor ("#151515");
	...
	UI.CreateButton(vert).SetText ("Play Card").SetColor (WZcolours ["Dark Green"]).SetOnClick (
		function ()
			...
			local strCrittersMessage = strPlayerName_cardPlayer.." sent Critters to " .. TargetTerritoryName ..", targeting "..assignToPlayerName;
