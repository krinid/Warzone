...
function Server_AdvanceTurn_End(game, addNewOrder)
	...
	for _, bonus in pairs(game.Map.Bonuses) do
		...
		if pID ~= WL.PlayerID.Neutral and getBonusValue(game, bonus.ID) ~= 0 and t[pID] ~= nil then
			...
			if not game.Settings.LocalDeployments then
				table.insert(t[pID], WL.IncomeMod.Create(pID, round(-getBonusValue(game, bonus.ID) + getBonusValue(game, bonus.ID) * sum(array)), "Dynamic Bonus for " .. bonus.Name));
			else
				table.insert(t[pID], WL.IncomeMod.Create(pID, round(-getBonusValue(game, bonus.ID) + getBonusValue(game, bonus.ID) * sum(array)), "Dynamic Bonus for " .. bonus.Name, bonus.ID));
			end
		end
	end
