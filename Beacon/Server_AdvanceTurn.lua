...
function Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
    if (order.proxyType == 'GameOrderPlayCardCustom' and startsWith (order.ModData, "Beacon|")) then
		...
		local priority = 8001; -- Smoke Bomb uses 7000, Phantom uses 8000 (default), so this reveals with priority over their fog; between 6000 and 8999 means it won't obscure a player's own territories

		terrs = getTerritoriesWithinDistance (game, targetTerritoryID, intBeaconRange);
		local fogMod = WL.FogMod.Create ('Revealed by Beacon', WL.StandingFogLevel.Visible, priority, terrs, nil);
		local event = WL.GameOrderEvent.Create (order.PlayerID, 'Placed a beacon', {});
...
function Server_AdvanceTurn_Start (game, addNewOrder)
	...
	if (arrFogModIDs [game.Game.TurnNumber] ~= nil) then
		local event = WL.GameOrderEvent.Create (WL.PlayerID.Neutral, 'Beacon effect dissipates', {});
