function Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
    if (order.proxyType == 'GameOrderPlayCardCustom' and startsWith (order.ModData, "Beacon|")) then
		...
		local priority = 7500; -- Smoke Bomb uses 7000, Phantom uses 8000 (default), so this reveals with priority over their fog but not over default Phantom visibility; between 6000 and 8999 means it won't obscure a player's own territories
