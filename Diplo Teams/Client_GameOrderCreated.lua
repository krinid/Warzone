...
function Client_GameOrderCreated (game, order, skip)
	...
	if (order.proxyType == 'GameOrderAttackTransfer') then
		...
		if (toowner ~= game.Us.ID and toowner ~= WL.PlayerID.Neutral and order.PlayerID ~= WL.PlayerID.Neutral and playersAreTeamMates (toowner, order.PlayerID, game, game.LatestStanding) == false and InWar (order.PlayerID, toowner) == false) then
