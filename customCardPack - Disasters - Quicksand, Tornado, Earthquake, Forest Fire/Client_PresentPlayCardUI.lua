...
function buttonSelectEarthquakeTargetBonusFromList (game, playCard, close)
	-- buttonEarthquakeSelectBonusFromList.SetInteractable (false);
	local bonusOptions = {};
	labelEarthquake_BonusTerrList.SetText ("");
	if (UI.IsDestroyed (vertEarthquakeBonusTerrList) == false) then UI.Destroy (vertEarthquakeBonusTerrList); end
	vertEarthquakeBonusTerrList = UI.CreateVerticalLayoutGroup (EarthquakeUI).SetFlexibleWidth (1);

	for bonusID, bonus in pairs (game.Map.Bonuses) do
		-- table.insert (bonusOptions, {text=bonus.Name, selected=function() EarthquakeTargetSelected(bonus); end});
		local strButtonText = bonus.Name.. " [" ..tostring (tablelength (bonus.Territories)).. " territories, income " ..tostring (getBonusValue (game, bonusID)).. "]";
		-- UI.Alert ("color " .. tostring (bonus.Color) .. " / " .. tostring (getBonusColor (game, bonusID)));
		-- UI.Alert ("bonusID=" ..tostring (bonusID).. "/" ..tostring (bonus.Name).. "\n" ..tostring (strButtonText));
		local horzBonusItem = UI.CreateHorizontalLayoutGroup (vertEarthquakeBonusTerrList).SetFlexibleWidth(1);
		-- UI.CreateButton (horzBonusItem).SetText ("").SetOnClick (function() EarthquakeBonusSelectFromList_selected ({ID = bonusID, Name = bonus.Name}, playCard, close) end); --.SetColor (0getBonusColor (game, bonusID));
		UI.CreateButton (horzBonusItem).SetText ("").SetOnClick (function() EarthquakeTargetSelected ({ID = bonusID, Name = bonus.Name}); end); --.SetColor (0getBonusColor (game, bonusID));
		UI.CreateLabel (horzBonusItem).SetText (strButtonText).SetColor (getBonusColor (game, bonusID));
	end
end
...
function play_Earthquake_card (game, cardInstance, playCard)
	...
	game.CreateDialog(function(rootParent, setMaxSize, setScrollable, game, close)
		...
		horzEarthquakeSelectButton = UI.CreateHorizontalLayoutGroup (EarthquakeUI).SetFlexibleWidth (1);
		buttonEarthquakeSelectBonus = UI.CreateButton (horzEarthquakeSelectButton).SetText ("Select Bonus").SetInteractable (false).SetOnClick (function () buttonEarthquakeSelectBonus.SetInteractable (false); Earthquake_SelectedBonusID = UI.InterceptNextBonusLinkClick (EarthquakeTargetSelected); end);
		UI.CreateLabel (horzEarthquakeSelectButton).SetText ("        ").SetColor ("#000000"); --blank line for spacing
		buttonEarthquakeSelectBonusFromList = UI.CreateButton (horzEarthquakeSelectButton).SetText ("[List]").SetColor (getColourCode ("minor heading")).SetInteractable (true).SetOnClick (function () buttonSelectEarthquakeTargetBonusFromList (game, playCard, close); end);
		...
		UI.CreateLabel (EarthquakeUI).SetText ("_").SetColor ("#000000"); --blank line for spacing
...
function EarthquakeTargetSelected (bonusDetails)
	...
	labelEarthquakeSelectBonus.SetText ("Bonus selected: " ..bonusDetails.Name.. " [" ..tostring (tablelength (EarthquakeGame.Map.Bonuses[bonusDetails.ID].Territories)).. " territories, income " ..tostring (getBonusValue (EarthquakeGame, bonusDetails.ID)).. "]");
	...
	for _, terrID in pairs(EarthquakeGame.Map.Bonuses[bonusDetails.ID].Territories) do
		...
		strLabelText = strLabelText .. "  " ..EarthquakeGame.Map.Territories [terrID].Name.."\n";
	end
	Game.HighlightTerritories(Game.Map.Bonuses[bonusDetails.ID].Territories);
	Earthquake_PlayCardButton.SetInteractable (true);
	buttonEarthquakeSelectBonus.SetInteractable (true);
	buttonEarthquakeSelectBonusFromList.SetInteractable (true);
	if (UI.IsDestroyed (vertEarthquakeBonusTerrList) == false) then UI.Destroy (vertEarthquakeBonusTerrList); end
